...
class VitsModel(TextToWaveform):
    def __init__(self, cfg: DictConfig, trainer: 'Trainer' = None):
        ...
        # Gradient clipping value for manual optimization (default: None = no clipping)
        self.grad_clip_val = cfg.get("grad_clip_val", None)
    ...
    def training_step(self, batch, batch_idx):
        ...
        norm_d = clip_grad_value_(self.net_d.parameters(), self.grad_clip_val)
        optim_d.step()
        ...
        norm_g = clip_grad_value_(self.net_g.parameters(), self.grad_clip_val)
        optim_g.step()
    ...
    def train_dataloader(self):
        ...
        # Check if batch_sampler is configured (for DDP/multi-GPU)
        if hasattr(self.cfg.train_ds, 'batch_sampler') and self.cfg.train_ds.batch_sampler is not None:
            ...
            # Single-GPU mode: use standard DataLoader without batch_sampler
            dataloader = torch.utils.data.DataLoader(
                dataset,
                collate_fn=dataset.collate_fn,
                **self.cfg.train_ds.dataloader_params,
