...
# Optional dependency/type: newer NeMo versions may expose a biasing request config.
# Keep it as an annotation-only type and fall back to Any if unavailable.
try:
    if os.environ.get("NEMO_DISABLE_ASR_CONTEXT_BIASING", "0") != "1":
        from nemo.collections.asr.parts.context_biasing.biasing_multi_model import BiasingRequestItemConfig
    else:
        BiasingRequestItemConfig = Any
except Exception:
    BiasingRequestItemConfig = Any
...
class Hypothesis:
    ...
    biasing_cfg: Optional[BiasingRequestItemConfig] = None
